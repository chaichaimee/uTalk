...
# Hard cap on any string reaching the synthesizer via this add-on.
# Guards against pathologically long user-entered translation strings
# (settingsPanel text fields have no length limit of their own).
_MAX_SPOKEN_MESSAGE_LENGTH = 200


def _capSpokenLength(text):
	if len(text) > _MAX_SPOKEN_MESSAGE_LENGTH:
		return text[:_MAX_SPOKEN_MESSAGE_LENGTH] + _("...truncated")
	return text
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def __init__(self):
		...
		self._settings_popup_timer = None
	...
	def script_announceCopy(self, gesture):
		try:
			...
			isBrowseModeVirtualSelection = bool(
				obj
				and hasattr(obj, 'treeInterceptor')
				and isinstance(obj.treeInterceptor, browseMode.BrowseModeDocumentTreeInterceptor)
			)

			if isBrowseModeVirtualSelection:
				# Browse mode selection is a virtual-buffer construct tracked
				# only inside NVDA; it has no corresponding real DOM/OS-level
				# selection in the underlying application. Forwarding the
				# native Ctrl+C here copies nothing, because the application
				# itself sees no selection at all (confirmed from log.txt:
				# NVDA announced "copy" but nothing reached the clipboard
				# while the selection was made with Shift+Arrow inside a
				# Chrome/YouTube page). Only api.copyToClip() actually works
				# for this case.
			...
			# Outside browse mode (editable fields, native controls) a real
			# OS-level selection exists, so always forward the native
			# gesture first and let the application perform its own copy -
			# this preserves any rich clipboard formats the app itself adds.
			core.callLater(0, gesture.send)
			selectedTextFound = bool(self._get_selected_text_robust(obj)) if obj else False
			logHandler.log.debug(f"uTalk: announceCopy selection detected: {selectedTextFound}")
			self._safe_speak("copy")
		except Exception:
			core.callLater(0, gesture.send)
			self._safe_speak("copy")
	...
	def _handle_tap(self):
		...
		try:
			if self._tap_count == 1:
				...
				import gui
				if self._settings_popup_timer and self._settings_popup_timer.IsRunning():
					self._settings_popup_timer.Stop()
				self._settings_popup_timer = core.callLater(300, gui.mainFrame.popupSettingsDialog, NVDASettingsDialog, settingsPanel.uTalkSettingsPanel)
